...
class ConsoleCompleter(Completer):
    def __init__(self, contacts: List[Contact] = [], notes: List[Note] = []):
        ...
        self.commands = {cmd: meta["description"] for cmd, meta in COMMANDS.items()}
    ...
    def get_completions(self, document, event):
        ...
        if command in self.command_settings:
            if (
                len(parts) == 2
                and not is_after_space
                or (len(parts) == 1 and is_after_space)
            ):
                if self.command_settings[command].get("second_arg_contact_name"):
                    yield from self.search_in_contacts_by_name(
                        current_word, start_position
                    )
    ...
    def first_word_completions(self, document):
        ...
        for cmd, meta in self.commands.items():
            if not word or cmd.lower().startswith(word):
                yield Completion(cmd, start_position=start_position, display_meta=meta)
    ...
    def search_in_tags_by_name(
        self, current_word: str, start_position: int, note_id: str
    ):
